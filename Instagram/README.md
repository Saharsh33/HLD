# Production-Grade System Design: Instagram
**Target Level:** Senior Software Engineer / Principal Architecture Review  
**System Type:** Read-Heavy Distributed Media & Social Graph Platform  
...
* **`POST /v1/media/upload-url`**
  ...
  * *Response:* `{ "upload_url": "[https://s3-accelerate.amazonaws.com/](https://s3-accelerate.amazonaws.com/)...", "media_id": "med_987654" }`
...
## 7. First-Principles Jargon Glossary

* **Write Amplification:** An architectural phenomenon where one logical write operation by a client necessitates multiple physical read/write operations within the underlying storage or caching infrastructure.
* **Fan-Out:** The process of distributing a single message, event, or data point from one source to multiple destination queues, caches, or databases.
* **Cache Stampede:** A system failure state occurring when high-concurrency read requests simultaneously encounter a cache miss for the same key, causing all requests to query the backend database concurrently.
* **Distributed Mutex (Mutual Exclusion):** A synchronization lock shared across multiple physical servers or microservice instances to ensure that only one node can execute a critical section of code at any given timeframe.
* **Pre-Signed URL:** A cryptographic URL generated by an IAM role granting temporary, time-bound read or write access to a specific object storage bucket without exposing permanent cloud credentials to the client.
* **Log-Structured Merge (LSM) Tree:** The underlying data structure used by Apache Cassandra. Instead of updating rows in place (which requires slow random disk I/O and B-Tree rebalancing), LSM trees buffer writes in RAM (Memtable) and sequentially append immutable logs to disk (SSTables), providing exceptionally high write throughput.
* **Eventual Consistency:** A consistency model guaranteeing that if no new updates are made to a given data item, all replicas will eventually converge to identical state over time, trading immediate global consistency for high availability and low latency.

...
- **Idempotent payment calls:** every payment request carries a client-generated `idempotency_key`, checked against Redis/Postgres before charging — a network retry must never double-charge a card (this is the same mechanism from your payment ledger doc — booking calls it as a subroutine, it doesn't reinvent it)
...
|---|---|
| **Virtual waiting room** | An edge-level queue that admits users into the real system at a safe, controlled rate instead of all at once |
| **Token bucket** | A rate-limiting algorithm that releases a fixed number of "tokens" (permitted requests) per second |
| **SETNX** | "Set if Not Exists" — a Redis command where only the first caller succeeds, all others are instantly rejected |
| **SELECT ... FOR UPDATE** | A SQL row-level lock — forces any competing transaction on that row to wait until the current one finishes |
| **Hot key** | One cache/database key getting so much traffic it overloads its single node while others sit idle |
| **CQRS** | Splitting read traffic and write traffic onto entirely separate systems, each optimized for its own pattern |
| **Authorization hold** | A temporary, uncaptured charge against a card that proves it's real/valid without actually taking the money |

---

## 10. Talking Points Checklist (for the actual interview)

- [ ] Explicitly state that this system switches consistency models — AP for browsing, CP for the lock moment — and explain why
- [ ] Walk through the two-layer lock (Redis `SETNX` → Postgres `FOR UPDATE`) unprompted
- [ ] Explain the virtual waiting room as **admission control**, not just rate limiting — it's shaping traffic before it reaches anything stateful
- [ ] Raise the "someone locks a seat without booking" abuse case yourself, and answer with the layered defense (auth, rate limits, abandonment tracking, payment pre-auth)
- [ ] Point out where this system calls into the payment ledger as a subroutine, rather than re-implementing idempotency and ACID logic from scratch